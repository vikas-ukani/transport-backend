...
const TOKEN_EXPIRES_IN_MINUTES = process.env
  .ACCESS_TOKEN_EXPIRES_IN_MINUTES
  ? parseInt(process.env.ACCESS_TOKEN_EXPIRES_IN_MINUTES)
  : 60; // fallback to 60 minutes if not set
  const ACCESS_TOKEN_EXPIRES_IN_MINUTES = 7 * 24 * TOKEN_EXPIRES_IN_MINUTES; // 7 days in minutes
...
export function createToken(userId) {
  ...
  const expireSeconds = ACCESS_TOKEN_EXPIRES_IN_MINUTES;
